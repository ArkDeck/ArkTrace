...
public struct TraceOfflineInspectionParserProvenance: Hashable, Sendable {
    ...
    package init(_ identity: TraceParserIdentity) throws {
        guard Self.safe(identity.name),
            Self.safe(identity.reportedVersion),
            Self.lowercaseHex(identity.upstreamRevision, count: 40),
            TraceOfflineInspectionProvenance.sha256(identity.binarySHA256),
            Self.safe(identity.adapterVersion),
            Self.safe(identity.buildRecipeVersion)
        else {
            throw TraceOfflineInspectionService.invalidResult(reason: "invalidParserIdentity")
        }
        name = identity.name
        ...
        buildRecipeVersion = identity.buildRecipeVersion
    }
...
public struct TraceOfflineInspectionProvenance: Hashable, Sendable {
    public let parser: TraceOfflineInspectionParserProvenance
    ...
    package init(parsed: ParsedTrace) throws {
        ...
        parser = try TraceOfflineInspectionParserProvenance(parsed.parser)
    ...
    package init(
        ...
        for issue in issues {
            let key = (
                ...
                issue.scope ?? "",
                issue.count ?? Int64.min
            )
            guard previous.map({ $0 <= key }) ?? true,
                identities.insert(
                    "\(key.0)\u{0}\(key.1)\u{0}\(key.2)"
                ).inserted
            else {
                throw TraceOfflineInspectionService.invalidResult(
                    reason: "dataQualityOrderMismatch"
                )
            }
            previous = key
        }
