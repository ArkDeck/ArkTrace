...
final class TraceOfflineInspectionServiceTests: XCTestCase {
    func testReportBindsExactSourceAndPublishesPathFreeProvenance() throws {
        ...
        XCTAssertEqual(report.provenance.parser.name, parser.name)
        XCTAssertEqual(report.provenance.parser.version, parser.reportedVersion)
        XCTAssertEqual(report.provenance.parser.binarySHA256, parser.binarySHA256)
        XCTAssertEqual(report.provenance.parser.upstreamRevision, parser.upstreamRevision)
        XCTAssertEqual(report.provenance.parser.adapterVersion, parser.adapterVersion)
        XCTAssertEqual(report.provenance.parser.buildRecipeVersion, parser.buildRecipeVersion)
    ...
    func testReportRejectsNonCanonicalQualityOrder() throws {
        let issue = TraceDataQualityIssue(
            category: .probeTruncated,
            scope: "thread.start_ts"
        )
        let descending = matchingFixture(quality: TraceDataQuality(issues: [
            TraceDataQualityIssue(
        ...
        XCTAssertThrowsError(try TraceOfflineInspectionReport(
            parsed: descending.parsed,
            metadata: descending.metadata,
            expectedSourceSHA256: descending.parsed.sourceSHA256,
            expectedSourceByteCount: descending.parsed.sourceByteCount
        )) {
            XCTAssertEqual(
                ($0 as? ArkTraceError)?.details["reason"],
                "dataQualityOrderMismatch"
            )
        }
    }
