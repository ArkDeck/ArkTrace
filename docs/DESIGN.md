...
- 20 项里 **12 项低于 chroma 下限**，也就是「读起来是灰的」；
- 2 项落在亮度带外；
- 相邻最近一对对**正常视觉**只有 ΔE 8.5（下限 15），对红绿色盲 **2.6**；
- 10 项对画布对比度不足 3:1。

新表是 OKLCH 上的等色相环：20 个等间距色相，每个色相取**它能承载最多彩度的那个亮度**——黄自然落在亮处、
蓝落在暗处，亮度多样性是这么来的，而不是硬拗出来的；slot 顺序再按跨步 9 重排，使相邻 slot 落在色轮两侧。
实测：亮度带、chroma 下限全过，相邻最近一对正常视觉 **ΔE 30**、红绿色盲 **15.6**。仍有 9 项对画布低于 3:1，
这条 WARN 被接受的唯一理由是颜色从不是唯一通道（AT-APP-011）：slice 上有 label，Inspector 里有名称。
...
一件尚未做的记在这里：深色模式。这 20 个值按浅色画布校验；深色画布按校验器要求应另有一组步进值，现在只是
同一组颜色画在深底上（仍好于上游那张表：后者在深色下 4 项 FAIL，这张 1 项）。
