...
> 配色回写（2026-08-19）：§13.5 换掉上游那张 20 色表 —— 分配规则仍逐位对齐上游，颜色改为 OKLCH 等色相环并按校验器实测（相邻对正常视觉 ΔE 8.5→30，色盲 2.6→15.6）
> 配色回写二（2026-08-24）：§13.5 上一条测错了对象 —— 「相邻最近一对」测的是 slot 相邻，而 hash 打散身份、任意两 slot 都可能贴在一起；按全部 190 对重测是 3.8 / 色盲 0.42（上一条那两个色盲数还各取了一种色盲：2.6 是红色盲、15.6 是绿色盲）。改为按 slot 实际命中率（{0,4,8,12,16} 占 74.3%）分层设计，identity 与 state 两张表**联合**优化（它们逐行交替），斑马纹改用 `alternatingContentBackgroundColors`（原先两个语义色同值，画出来是零差异 —— 这同时把画布从 2 个变成 4 个，配色必须对四个都过 3:1），校验器落地为 `scripts/verify_palette.py` + `scripts/test_palette_verifier.py` 并进 CI contracts lane
> 功能回写二（2026-08-19）：§14.2 sidebar 进程名可点击跳转 —— 隐藏的泳道先显示，滚动目标取自布局完成后的 snapshot 而非估算偏移
> 缺陷回写四（2026-08-19）：§14.1 侧栏泳道名改为换行显示 —— 切尾正好切掉 `processName · threadName` 里唯一能区分两条泳道的部分
> 功能回写三（2026-08-19）：§14.2 搜索拆成两个界面 —— 侧栏按需展开的过滤框只管进程名/PID，工具栏的搜索不再匹配进程；被排除的域在 query 层就不发出
> 功能回写四（2026-08-19）：§14.2 两个搜索框各自有快捷键（⌘F / ⇧⌘F），改由 AppKit 接管焦点 —— SwiftUI `@FocusState` 扛不住菜单命令；同一处顺手关掉了输入框的 focus ring
...
配色不是主题，而是数据编码：SmartPerf Host 不按事件种类上色，而是把事件身份散列进一张固定调色板，因此同一个函数在不同 track、不同 trace、不同 session 里保持同一颜色。ArkTrace 复用被 pin 住的上游 parser，也复用这套**分配机制**——在上游同色的两个 slice 在这里也同色。

**分配是上游的，色值是 ArkTrace 的。** 这条分界要说清楚，因为它变过两次：2026-08-19 换掉了 20 色表，2026-08-24 换掉了 thread state 表。两次都只动色值，没有动任何一个「哪个事件落哪个 slot」的规则。

移植自 `source-lock.json` 所 pin 的上游 revision（Apache-2.0）：

| 上游实现 | ArkTrace 用它做什么 | 色值来自 |
|---|---|---|
| `ColorUtils.hash` / `hashFunc`（去掉数字后再 hash） | named slice 与 counter series 按名称落 slot | — |
| `ColorUtils.colorForThread` / `colorForTid` | CPU slice 按所属 process（无 pid 时退回 tid）落 slot | — |
| `ColorUtils.FUNC_COLOR_B` 的**位置**（20 槽） | hash 配色的取值域大小 | **ArkTrace**（改于 2026-08-19） |
| `Utils.getStateColor` 的**链**（哪些拼写共用一个填充、哪些落 catch-all） | thread state 取色 | **ArkTrace**（改于 2026-08-24） |
| `ColorUtils.JANK_COLOR` | frame lane 按 jank flag 取色 | 上游 |
...
**20 色表换成 ArkTrace 自己的**（改于 2026-08-19）。此前整套颜色逐字节取自上游 `ColorUtils.FUNC_COLOR_B`，
理由是「同一个进程在两个工具里同色」。代价一直挂在眼前：真机 overview 是一片橄榄绿与灰紫的糊。这次不是凭
感觉换，而是先量——用调色板校验器（OKLab ΔE、色觉缺陷模拟、对比度）测上游那张表：

- 20 项里 **12 项低于当时设的 chroma 下限（0.10）**，也就是「读起来是灰的」；
- 2 项落在亮度带外；
- 相邻最近一对对**正常视觉**只有 ΔE 8.5（下限 15），对红色盲 **2.6**、绿色盲 4.2；
- 9 项对画布对比度不足 3:1（早先此处写 10，实测是 9）。

那一版的新表是 OKLCH 上的等色相环：20 个等间距色相，每个色相取它能承载最多彩度的那个亮度；slot 顺序按跨步 9
重排。报出的成绩是「相邻最近一对正常视觉 ΔE 30、红绿色盲 15.6」。

**没有动的是分配**：hash、去数字、depth 传 0、`colorForTid` 的 pid→tid 回退全部照旧，所以「两个事件在上游
同色则在这里同色」这条关系仍然成立，只是那个颜色变了。palette 测试相应改为钉 **slot** 而不是钉颜色——slot 才是
上游对齐的那个事实。这一条至今有效。

### 按命中率重做（改于 2026-08-24）

上一版测错了对象，而且校验器没有入库，所以没人复算。

**「相邻最近一对」不是有效指标。** slot 相邻不是屏幕相邻：分配 hash 打散身份，任意两个 slot 都可能贴在一起。
按真正会同屏出现的**全部 190 对**重测，那张表是正常视觉 **3.8**、红绿色盲 **0.4**；它所取代的上游表是 0.3 / 0.3。
在它声称要修的那条轴上，实际收益是 0.3 → 0.4。同时平均彩度从 0.105 涨到 **0.154**，亮度带宽从 0.255 涨到 0.282。

**更要紧的是，hash 根本到不了大半张表。** 上游 hash 每轮在 `Double` 域内乘 16777619、之后才截回 Int32；乘积
需要 55 位而 double 只有 53 位，低两位在截断前就被舍掉了。于是 `|hash|` 大约四分之三是 4 的倍数，而 20 = 4×5，
`% 20` 塌缩到 {0,4,8,12,16}。实测（`verify_palette.py` 的确定性语料：2 万个合成 slice 名 + pid 1..19999）：

| slot | 命中率 |
|---|---|
| 0 / 4 / 8 / 12 / 16 | 各约 14.9%，合计 **74.3%** |
| 2 / 6 / 10 / 14 / 18 | 各约 2.6%，合计 12.9% |
| 十个奇数 slot | 各约 1.3%，合计 12.8% |

**五个颜色就是整条时间线的配色**，而那一版恰好把它们选成了最刺眼的组合：`#2b4fb0` 深蓝、`#07c3c1` 电光青、
`#87a303` 橄榄、`#c95c03` 焦橙、`#8c2978` 品红——五个近色域边缘的等分色相（相距约 72°），彩度 0.153，亮度在
0.46↔0.74 之间来回弹。这不是「等色相环」的副作用，而是它的直接后果：max-chroma-per-hue 这条规则在优化彩度
的同时，因为 sRGB 色域尖点随色相移动，必然把亮度方差也一起最大化。

新表按这三档设计，**分配规则一个字未动**：

- **Tier A**（0/4/8/12/16，74.3%）：一个和谐族，亮度带宽 **0.038**、彩度 **0.088**，色相走
  蓝→青→鼠尾草→古铜→陶土 的受控扫描。占四分之三像素的东西必须读成一个面，而不是五盏警示灯。
- **Tier B**（2/6/10/14/18，12.9%）：介于 A 与 C 之间。它**不是**「同族下一档亮度」——整张表被压在一条很窄的
  亮度带里（见下），根本没有「下一档」可走；B 是绕色环的第二圈，彩度略高。
- **Tier C**（十个奇数 slot，各 1.3%）：「稀有」只在**按名称散列的 slice** 上成立。按泳道不成立：一条 trace 有
  6–30 个进程而不是两万个，所以整宽的 CPU 泳道经常落在 Tier C。它的彩度因此被压到接近 Tier A（实测 0.086 对
  0.088），而不是任其放飞。
- **slot 5 回到中性。** `hash("0") == 5`，而 `processColor` 取 `pid ?? 0` / `tid ?? 0`，所以「既无 pid 又无 tid」
  的 CPU slice 全落这里。它原本是上游的 `#8a8a8b`（彩度 0.0015），2026-08-19 那次「12 项低于 chroma 下限」的
  修正把唯一一个「灰是有意义的」项也一起修成了饱和酒红。`greyColor` 同理，从 `#F0F0F0`（对白底 1.14:1）改为同
  一个中性色——两者都是「不知道这是谁的」，一个颜色就够。

**画布是四个，不是两个。** `windowBackgroundColor`／`controlBackgroundColor`／`textBackgroundColor` 确实在
Aqua 下都是 `#FFFFFF`、DarkAqua 下都是 `#1E1E1E`；但下面那条斑马纹修复让 `drawTracks` 用
`alternatingContentBackgroundColors` 铺奇数行，于是又多出 `#F4F5F5` 与 `#292929`。一条 track 落在奇数行还是
偶数行只是排序的偶然，所以每个填充必须对四个都过 3:1。这不是假想：斑马纹修复和本次配色的第一版是一起做的，
那一版有六个填充对 `#F4F5F5` 落在 2.78:1–2.96:1 之间——其中两个是最常见的 thread state——而校验器报的是
0/20，因为它当时只知道 `#FFFFFF`。

**深色模式的债一并清掉，而且不需要第二张表。** 四条要求在同一个相对亮度区间上交汇：对 `#F4F5F5` ≥ 3:1 要求
Y ≤ 0.270，对 `#292929` ≥ 3:1 要求 Y ≥ 0.167，**黑色** label ≥ 4.5:1 要求 Y ≥ 0.175，另外两个画布比这两个更松。
两张表全部 28 个填充都落在 Y ∈ [0.180, 0.265]。白色 label 需要的是 Y ≤ 0.183 —— 一个几乎不相交的区间，混用两种
墨色正是上一版 label 前景色在 19 步里翻转 13 次的原因；选定一个区间之后，翻转是被消掉而不是被调掉的。
...
|---|---|---|
| 命中率加权彩度 | 0.154 | **0.088** |
| 全部 190 对最小 ΔE | 3.8 | **5.4** |
| Tier A 亮度带宽 | 0.279 | **0.038** |
| label 墨色翻转 | 13/19 | **0/19** |
| 对四个画布中任一个 < 3:1 | 17/20 | **0/20** |
| 最差 label 对比度 | 5.02:1 | 4.60:1 |

**那条窄亮度带也是代价，得说清楚。** 28 个填充压进一条窄亮度带，只剩色相与彩度两个维度可用。在同一条带里做
无约束装填，28 色的上限约 ΔE 6.6；5.4 是在此之上再要求 Tier A 成族、state 保住语义、slot 5 必须是灰之后剩下的。
这套颜色没有一个是「一眼认定」的，这正是下一段的意思。

**一件说清楚而不是绕过去的事：20 个身份色在任何彩度下都做不到色盲可分。** 等明度、10 色×2 明度、7 色×3 明度
都试过，deutan 最小 ΔE 全部落在 0.2–1.4。实测全部 190 对的 deutan 最小值：现表 **0.97**、2026-08-19 那版 0.42、
上游 0.33 —— 相对提升 2–3 倍，但可用性要到 ~15 才开始，所以这是改善不是修复。这是 20 个类别本身的上限，不是
配色的失误。AT-APP-011 要求颜色永不作为唯一通道正是为此，身份精度由 label、Inspector 与搜索承担 —— 但这条豁免
只有在第二通道真的存在时才成立，见下面 density band 与斑马纹两条。

**校验器入库。** 上一版引用的「调色板校验器」从未提交，所以一个测错对象的指标活过了 review。现在它是
`scripts/verify_palette.py`：直接从 `TimelineColorPalette.swift` 读表（不复制任何色值）、重算**现表**的每一个
数字、任一项退化即 fail，跑在 CI 的 contracts lane 里。它管不到上表「2026-08-19」那一列——那张表已不在任何它
读得到的源码里，所以那些数字是历史而不是不变量。配套的 `scripts/test_palette_verifier.py` 对校验器本身做变异
测试（14 个用例，各按一种方式破坏配色并断言 gate 拒绝），跑不出 fail 的 gate 不是 gate。校验器同时把自己的
hash 移植与 `TimelinePaletteTests` 里钉住的上游向量对表——否则「74.3%」只是它在验证自己。这条对表当场抓到一个
真 bug：Swift 迭代 `value.utf16`，而移植迭代的是 code point，含 emoji 的名字算出来不一样。
`ci_plan.sh` 为此单独给 `TimelineColorPalette.swift` 加了一条规则——改它要同时跑两条编译 lane 和 contract lane。

### thread state 表用同一体系重述（改于 2026-08-24）

上面那句「真机 overview 是一片橄榄绿与灰紫的糊」点名的两个颜色，从来不在 20 色表里：`R` 是 `#a0b84d`（橄榄
绿）、`I` 是 `#673ab7`（灰紫），都在 `Utils.getStateColor` 的移植表里，而 `git log --all --grep=color -i` 显示
**没有任何一次提交动过它**。2026-08-19 换掉的是另一张表。

thread state lane 大约占全部行数的一半（`TraceDocumentController` 每个线程 append 一条 `.threadState` 加一条
`.namedSlice`），所以它铺的画布和身份调色板一样多，而且两者逐行交替。此前的问题不止「两套视觉语言」：

- **层级是反的。** 吵的是罕见项，常见项反而看不见：`S`（sleeping，任何 capture 里最常见的状态）对 `#FFFFFF`
  只有 **1.32:1**，`R` 2.22:1，`R-B` 1.72:1，而罕见的 `I` 是 7.33:1。
- **没有一项能同时活在两种外观下。** `#e0e0e0` 在白底上是隐形的，`#673ab7` 在近黑底上是隐形的。

重述保留上游的**分组**（哪些拼写共用一个填充、哪些落 catch-all，`TimelinePaletteTests` 钉的就是这个），把色值
换成一条有语义形状的坡道——常见且无趣的状态安静，要找的东西才有彩度：

| state | 填充 | 角色 |
|---|---|---|
| `S` sleeping | `#898d94` | 最常见，近中性，后退 |
| `I` idle | `#627987` | 更安静，略偏冷 |
| `R` / `R+` runnable | `#7b7849` | 想要 CPU 而没拿到 |
| `R-B` | `#948c63` | `R` 族，亮一档 |
| `Running` | `#489252` | 读者真正在找的状态 |
| `D*` io wait | `#bd7211` | 暖色信号 |
| `D-NIO` / `DK-NIO` | `#a47c74` | `D` 族里安静的那个 |
| catch-all | `#d85d72` | 两张表里彩度最高的一项 |

catch-all 是唯一允许「吵」的：无法识别的调度状态是数据问题，数据问题不该融进墙里。校验器为它钉三条：必须是
**两张表里**彩度最高的一项、与 `S` 至少 ΔE 15（实测 16.0）、且与**任何**其它填充至少 ΔE 7（实测 8.6）。第三条
是第一版漏掉的：只对着 `S` 检查，钉的是本来就不可能撞的那一对，而警示色实际待的是挤满暖色的那一段——第一版
它距 Tier A 的 `#b56d63` 只有 ΔE 5.7。

**两张表是联合优化的。** 它们逐行交替出现，分开优化正是第一版让状态填充离 slice 填充只有 ΔE 4.9 的原因。现在
表内最小 ΔE **6.2**，跨表最小 ΔE **5.4**。5.4 让状态泳道**看得出来**不是 slice 泳道；它不会让两者成为两种视觉
语言，压在一条窄亮度带里的任何配色都做不到，别在文档里把它说成做得到。

校验器还钉一条派生事实：上游的链必须恰好解析出 **7** 个不同填充。把两个状态合并成同色，在去重之后会从每一项
ΔE 测量里**消失**而不是让某一项 fail——这是字面钉值表达不了、只能由 gate 表达的东西。

### 斑马纹此前是个 no-op（改于 2026-08-24）

`drawTracks` 用 `textBackgroundColor` 与 `controlBackgroundColor.withAlphaComponent(0.45)` 交替铺行底。这两个
语义色在本平台同值（Aqua 都是 `#FFFFFF`，DarkAqua 都是 `#1E1E1E`），0.45 的白叠在白上仍是白 —— **两种外观下
都是零像素差异**。也就是说给画布提供横向结构的那层东西一直没有画出来，分隔线又只有 0.098 alpha，于是组织整个
视图的责任全压在填充色上，它当然只能靠喊。改用 AppKit 为此提供的 `alternatingContentBackgroundColors`
（Aqua `#FFFFFF` / `#F4F5F5`，DarkAqua `#1E1E1E` / 4.7% 白），索引 1 在深色下是半透明的，依赖 `draw` 已经铺好
的窗口底色。

**尚未做的记在这里：** jank 表（`#42A14D` / `#FF651D` / `#E8BE44` / `#C0CE85`）仍是上游值，其中 `#E8BE44` 对
`#FFFFFF` 只有 1.77:1、`#C0CE85` 只有 1.69:1。校验器会打印这张表的实测值但不 gate 它，原因写在脚本里。
`THIRD_PARTY_NOTICES.md` 仍称移植「covers `ColorUtils.FUNC_COLOR_B` … and `Utils.getStateColor`」，这在两张表
都换成 ArkTrace 自己的值之后是过度归属（不是许可问题，Apache-2.0 下多归属无害），改它需要同时重钉
`scripts/verify_licenses.sh` 的字节数与 SHA、以及 `scripts/verify_phase5_cli_distribution.py` 的
...
**折叠状态也持久化，但只持久化用户自己的选择**（改于 2026-08-19）。`inspectorVisible` 存的是「用户上一次
对『Inspector 该不该显示』的回答」，只有显式的折叠/展开会写它；自动折叠不写。区别不是洁癖：自动折叠说的是
这一个窗口此刻多宽，把它带到下一次启动，就会在一个完全放得下的窗口里藏起一块没人要求藏的 pane。

窗口实际的显隐仍是 per-window 的 `@State`，只在建窗时从偏好取初值。若直接把 `@AppStorage` 当作显隐本身，
两个不同尺寸的窗口会互相打架：窄窗口自动折叠 → 共享值变 false → 宽窗口的 pane 跟着消失 → 宽窗口自己的
布局回合发现放得下又展开 → 窄窗口再折叠，来回不停。
...
**侧栏的泳道名换行，不截断**（改于 2026-08-19）。一行是 `checkbox + 标题 + 深度三角 + pin`，标题夹在中间，
宽度不够时 SwiftUI 默认切尾，读者看到的于是是 `kworker/2:2 · kwor…`。问题不在于"少看了几个字"：
`processName · threadName` 的辨识信息几乎全在尾部——同一进程下几十条泳道共享同一前缀，线程名本身又已被
内核裁到 15 字符——切掉的正是唯一能区分两条泳道的部分，这样一行名字指不出任何一条泳道。

改法是让标题重排：`lineLimit(nil)` + `fixedSize(horizontal: false, vertical: true)`，宽度不够就占两行；
两个修饰符缺一不可，只给 line limit 仍会被父布局压回一行。进程分组的标题同样处理。侧栏宽度可拖、字号
可变，AT-APP-003 本来就不许文字 pane 依赖固定宽高——能换行的标题才满足这条，中间截断（`truncationMode`）
只是把同一个问题换个位置切。
...
**两个搜索框，两个问题**（改于 2026-08-19）。侧栏顶上多了一个放大镜按钮，按下展开一个过滤框，只匹配
**进程名和 PID**，把泳道树收窄到匹配的进程；工具栏那个搜索框相应地不再匹配进程，只找 **TID / 线程名 /
slice 名**。

拆开的理由不是"两个框比一个多"，而是它们本来就是两个问题。「哪个进程」问完就留在侧栏——找到它、按一下
名字跳到它的泳道（见下），全程不需要动 timeline；「什么时候发生了什么」才需要跳到某个时刻。混在一个框里
时，真机 trace 上一个名字往往同时是进程名和 slice 名，结果列表会被同名进程占满，而那些进程行的作用只是
"跳到它的泳道"——侧栏本来就在做的事。

实现上有两点值得记：

搜索域进 request，不在结果里过滤。`TraceViewerSearchRequest.Domains` 是一个 OptionSet，被排除的域**根本
不发查询**——按结果过滤会把省下来的那一半代价照付一遍，而 process/thread 目录查询正是一次搜索里最贵的部分。
`ipid:` 这种显式身份查询跟着 `.process` 一起关掉：它问的是进程。

侧栏的过滤在内存里做，按**行上显示的那串文字**匹配（进程行是 `name [pid]`）。一次 contains 就同时覆盖了
名字和 PID，不需要第二套规则，读者看到什么就能搜到什么；能跳过去的进程也只有树里这些，所以拿仓库再查一遍
既慢又会给出跳不过去的结果。过滤框关闭时一定把文字清空——一个正在藏进程、而输入框已经收起来的过滤器，
就是下一份 bug 报告。

**⌘F / ⇧⌘F，以及为什么这两个输入框是 AppKit 的**（改于 2026-08-19）。两个搜索各给一条快捷键：⌘F 打开并
聚焦侧栏的过滤框，⇧⌘F 聚焦工具栏的搜索框。一条 ⌘F 去猜用户想要哪一个，是猜错一半的设计。

写法上原本是最直白的那种：菜单命令改一个 `@FocusState`。在真机上验证的结果是**它根本不生效**——命令执行了
（过滤框确实展开了），但键盘没过去，字打进了画布。把请求推迟一个 runloop、先 `makeFirstResponder(nil)` 让
画布松手、甚至在输入框本来就开着的情况下再按一次，都一样：SwiftUI 的焦点在菜单命令这条路径上根本没接管
窗口。改成 AppKit 之后一次就成——`window.makeFirstResponder(field)` 返回 true，first responder 变成该
输入框的 field editor，并在 500 ms 后仍然是它。

所以两个输入框都换成了同一个 `FocusableTextField`（`NSViewRepresentable` 包 `NSTextField`），聚焦请求沿用
本仓库已有的形状：controller 上一个只增不减的 request id，`updateNSView` 里比对、变了就在下一个 runloop
`makeFirstResponder`——和 `TimelineView`、`InspectorFocusButton` 的做法一致。用计数器而不是 `Bool` 的理由
是「再按一次 ⌘F」必须还能聚焦，而需要有人记得放下的标志位，迟早没人放下。

顺带解决了另一件事：`focusRingType = .none`。输入框的焦点本来就由**插入点**表达，它一直在闪、不靠颜色；
再套一圈蓝框只是把同一件事喊一遍，而搜索框在一次搜索里几乎全程处于焦点状态。这条只适用于有插入点的输入
框——按钮和画布仍然保留自己的可见焦点指示（时间轴那条 `:focus-visible` 规则见 §14.1）。

**泳道树有界这件事，只说一次**（改于 2026-08-19）。此前每个进程名旁边挂一个 `⋯` 表示"该组被截断"，而这个
标记的来源是建树时那次 `ThreadQuery(limit: 1_000)` —— 目录一被截断，**每个**进程组都被标成 truncated。于是
几百行里重复着同一个事实，读者要么当它是装饰，要么误读成"这个进程有问题"。现在改成树末尾的一行说明，说的
是它真正的意思：这棵树没有列全。

**按进程名跳到它的泳道**（改于 2026-08-19）。侧栏里的进程名此前只是 `DisclosureGroup` 的标签，点它只会展开
一个列表——而「想看这个进程」时按下它的名字是最自然的动作。现在名字是一个按钮：泳道若全部隐藏则先显示
（跳到画不出来的东西不算跳），然后 timeline 纵向滚动到该组第一条泳道；三角形仍然只管侧栏列表的展开收起。

两个实现约束值得记住：

- **滚动目标必须等布局。** 泳道的 y 只存在于已经排布好的 snapshot 里，隐藏的泳道根本不在其中。所以
  controller 先改可见性、再等下一份 snapshot 落地，然后才发出滚动请求（`TimelineScrollRequest`，带单调 id，
  这样对同一处连点两次仍然会滚）。凭 track 序号估一个偏移量看似省事，但深度行数、collapsed 状态和
  density/detail 的行高都会让它算错。
- **显示泳道不得顺带展开调用栈深度**：`showsNestedDepth` 原样保留，这是用户自己的选择。
