...
2026-08-12 的 clean gate 实测结果：81 tests、0 failed、0 skipped。Gate 在执行测试前验证 binary、manifest、architecture、fixture/license 的 SHA-256、byte count 与 Git blob OID；任一缺失或漂移都会 fail closed。
...
| AT-PARSE-002/005 | async snapshot/hash/Mach-O/manifest validation；actual binary、manifest、locked evidence 与 `metadata.parser` 强绑定 |
...
| AT-PARSE-009 | TERM → 500 ms grace → same-PID KILL；wait/reap；cancel/promotion gate；device/inode-owned rollback；无 orphan/Ready partial |
| AT-PARSE-010 | upstream 模糊失败稳定映射为 `TRACE_PARSE_FAILED`；不伪造具体格式原因 |
| AT-DB-002～005 | real schema v2 fingerprint；declared affinity/storage class；bounded table/range/relationship/quality probes；typed fail-closed errors |
| AT-DB-009 | `arktrace_v1_*` indexes；真实 scheduling/state/slice fixture 的 `EXPLAIN QUERY PLAN` 命中目标 index |
...
|---|---:|
| ArkTraceCoreTests | 7 |
| ArkTraceParserTests | 27 |
| ArkTraceStoreTests | 36 |
| ArkTraceIntegrationTests | 11 |
| 合计 | 81 |
