...
**Sidebar 里的进程名必须可点击并跳到它的泳道**：按下进程名，其泳道若被隐藏则先显示出来（跳到画不出来的
东西不算跳），随后 timeline 纵向滚动到该组第一条泳道。滚动目标必须取自**已完成布局的 snapshot**，不得凭
估算的偏移量跳；展开泳道不得顺带把用户折叠过的调用栈深度重新展开。三角形仍然只管展开/收起侧栏里的列表。
...
**泳道树是有界的这件事必须说，且只说一次。** 泳道树建立在有界的线程/series 目录之上，目录被截断时整棵树
都不完整——这是**一棵树的属性，不是某个进程的属性**。因此不得在每个进程名旁边挂一个标记：几百行重复同一
个事实，读者只会当作装饰，而且它看上去像在说「这个进程有问题」。必须在树的末尾用一行文字说明还有泳道
未列出，并且这行文字本身也要能完整显示。

**Sidebar 里的泳道标题不得被省略号截断**：宽度不够时必须换行显示完整名字。`processName · threadName`
的辨识信息几乎全在尾部——同一进程下几十条泳道共享同一前缀，且线程名本身已被内核裁到 15 字符——截尾
等于把标识丢掉（`kworker/2:2 · kwor…` 指不出任何一条泳道）。这与 AT-APP-003「文字 pane 不得依赖固定
宽高」是同一条要求：侧栏宽度可变、字号可变，标题必须随之重排而不是被切掉。
...
搜索分两个界面，各自回答一个问题，且**不得互相作答**：

- **Sidebar 的进程过滤**（按需展开的搜索框）只匹配**进程名与 PID**，只收窄侧栏的泳道树，不动 timeline；
- **Toolbar 的搜索**只匹配 **TID、thread name、slice name**，结果选择后跳转并在可用 detail LOD 下高亮
  真实 event/track。

两者合起来仍须覆盖 PID、TID、process name、thread name、slice name。分开的理由是它们本就是两个问题：
「哪个进程」用完就留在侧栏，「什么时候发生了什么」才需要跳时间轴；混在一个框里时，进程名同时也是 slice
名的 trace 会让结果列表被同名进程挤满。被排除的域**不得被查询**——省下的正是那一半查询的代价。

Sidebar 的过滤按**它显示的那行文字**匹配（进程行即 `name [pid]`），不引入第二套匹配规则；过滤只是筛选，
不得改变泳道的显隐或折叠状态，且不得随 trace 一起留存到下一次打开。

两个界面各自有键盘入口：**⌘F 聚焦 sidebar 的过滤框**（折叠时先展开），**⇧⌘F 聚焦 toolbar 的搜索框**；
两条都必须出现在菜单里，不得只做成隐藏按键。菜单发出的聚焦请求必须真的落到输入框上——「命令执行了但键盘
没过去」等于没有这条快捷键，实现层的注意事项见 DESIGN §14.2。没有 trace 时两条都必须禁用。

**输入框获得焦点时不画 focus ring**：文本框的焦点由**插入点**表达——它一直在闪、不依赖颜色，已满足
AT-APP-009/AT-APP-011 对「焦点有清晰可见指示」的要求；再套一圈高亮蓝框只是把同一件事喊一遍，而搜索框在
一次搜索里几乎全程处于焦点状态。这条只针对文本输入框：没有插入点的控件（按钮、可聚焦画布等）仍须保留
各自的可见焦点指示。
