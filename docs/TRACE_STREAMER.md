...
2. `ThirdParty/TraceStreamer/patches/faultloggerd-apple-clang.patch` 是独立、可审计且 fail-closed 应用的本地补丁；它把 Apple clang 拒绝的 pointer → `Elf32_Addr` 截断改为经 `uintptr_t` 的显式转换；
...
`trace-streamer-4.3.7-e0167fbb13bf` 下载 asset `trace_streamer-macos-arm64`，并在赋予执行位及运行前校验：

- SHA-256：`e0167fbb13bf666dd589c7b27d697683bec2762ec66cefc935139e6da49ecbbf`；
- 架构：`arm64`；
- 上游 revision：`447a0a49a7b3b914d6e9bd00648ba5a340f6fbf6`；
- build recipe：`e4fec8cc9cbb1be13748e7149424ce664a545c2296b424b6ff520cc3e84d3f06`。
